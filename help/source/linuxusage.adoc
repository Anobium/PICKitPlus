...
This is the operations to use the executable.&#160;&#160;We assume a PICkit programmer and you are using the programmmer as the power source.&#160;&#160;
{empty} +
{empty} +
...
The application can be forced to use a PICKit 3 programmer.
...
{empty} +
{empty} +
`-w -pPIC12f675   -f12f675.hex  -u343c  -mpec  -a4.5 -zv`
{empty} +
{empty} +
...
{empty} +
{empty} +
Where a clone programmer with fixed voltage CANNOT change the operating voltage.&#160;&#160;
You could use the PICKit Plus GUI to show fixed voltage constraint within the programmer by trying to change the operating voltage.
&#160;&#160;the application cannot change the operating voltage if the PICKit Plus GUI cannot.

<|-b
<|Requires a filename as parameter
<|-b&#160;&#160;&#160;&#160;The  filename of the PKPlusDeviceFile.dat file.

The is an optional switch.  The PKPlusDeviceFile.dat file is assumed to be in the same folder as the application.exe.
...
This switch will delay the exit of the application.  This enable you to review the output from the application.

You can specify a time delay or wait for a key press.  The options are -dN or -dK. Where N is an integer value.

When using an IDE ensure the IDE supports -dK.  Using -dK with some IDEs that does not support user input during programming may cause the IDE lock waiting for a key press that cannot passed the application.
<|Usage:
{empty} +
{empty} +
`-d1`&#160;&#160;Delay 1 second
