...
When integrating this executable with other applications the following details are a working example.

PKCMD-PI Problem when running inside pyqt5 / QtProcess application.

Passing a string list of parameters to the QProcess with the switch and corresponding parameter in one string with space between them, the correct way is to pass them as two independent strings without spaces.
